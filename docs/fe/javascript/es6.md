...
log('tongren', 1, 2) // tongren [1, 2]
log('tongren', 1, 2, 3) // tongren [1, 2, 3]
```
...
// 字符串会以字符数组的形式做合并
Object.assign({}, 'tongren') // {0: 'm', 1: 'a', 2: 'o', 3: 'm', 4: 'a', 5: 'o'}
...
```js
const obj = { name: 'tongren', age: 18 }
Object.keys(obj) // ['name', 'age']
Object.values(obj) //  ['tongren', 18]
Object.entries(obj) // [['name', 'tongren'], ['age', 18]]
```
...
```js
Object.fromEntries([['name', 'tongren']]) // {name: 'tongren'}

/* Map 转对象 */
const map = new Map([['name', 'tongren']])
Object.fromEntries(map) // {name: 'tongren'}

/* 将查询字符串转为对象 */
const params = 'name=tongren&age=18'
Object.fromEntries(new URLSearchParams(params)) // {name: 'tongren', age: '18'}
```
...
const a = 0 ?? 'tongren' // 0
const b = '' ?? 'tongren' // ''
const c = null ?? 'tongren' // 'tongren'
const d = undefined ?? 'tongren' // 'tongren'

/* || 运算符 */
const A = 0 || 'tongren' // 'tongren'
const B = '' || 'tongren' // 'tongren'
const C = null || 'tongren' // 'tongren'
const D = undefined || 'tongren' // 'tongren'
```
...
- **`||` 运算符**只要左侧是 [假值](/fe/javascript/types#类型转换) 就会返回右侧的值
