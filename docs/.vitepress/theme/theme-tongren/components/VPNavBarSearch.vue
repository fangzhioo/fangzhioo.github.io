...
const buttonText = computed(() => theme.value.algolia?.locales?.[localeIndex.value]?.translations?.button?.buttonText || theme.value.algolia?.translations?.button?.buttonText || 'Search')
...
<template>
  <div v-if="theme.algolia" class="VPNavBarSearch">
    ...
    <div v-else id="docsearch">
      <button type="button" class="DocSearch DocSearch-Button" aria-label="Search" @click="load">
